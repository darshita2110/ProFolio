...
class FirebaseAuthService implements IAuthService {
  ...
  void _handle(FirebaseAuthException e) {
    switch (e.code) {
      case 'weak-password':           
        throw AuthException(message: 'Password must contain uppercase, lowercase, number, and special character');
      case 'email-already-in-use':    
        throw AuthException(message: 'Email already in use');
      case 'invalid-email':           
        throw AuthException(message: 'Invalid email format');
      case 'user-not-found':          
        throw AuthException(message: 'No account with that email');
      case 'wrong-password':
      case 'invalid-credential':      
        throw AuthException(message: 'Incorrect email or password');
      case 'too-many-requests':       
        throw AuthException(message: 'Too many attempts — try later');
      case 'network-request-failed':  
        throw AuthException(message: 'Network error. Check connection');
      default:                        
